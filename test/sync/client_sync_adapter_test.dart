...
void main() {
  ...
  test(
    'loadDirtyDocs ne retourne que les lignes isDirty, avec le bon mapping de champs',
    () async {
      await database
  ...
  test(
    'loadDirtyDocs inclut le profil étendu (email, adresse, type, notes)',
    () async {
      await database
          .into(database.clients)
          .insert(
  ...
  test(
    'applyRemoteDocs peuple une ligne absente en local (nouvel appareil)',
    () async {
      await clientsCollection().doc('remote-1').set({
        'name': 'Client distant',
        'phone': '221770000000',
  ...
  test(
    'applyRemoteDocs écrase une ligne locale dont le distant est plus récent',
    () async {
      await database
          .into(database.clients)
          .insert(
  ...
  test(
    'applyRemoteDocs propage une suppression distante (soft delete)',
    () async {
      await database
          .into(database.clients)
          .insert(
