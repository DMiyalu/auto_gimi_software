...
void main() {
  ...
  test(
      'createPrestationForImmatriculation crée un véhicule minimal si '
    ...
    final vehicule = await repository.watchVehicule(prestation.vehiculeId).first;
  ...
  test(
      'createPrestationForImmatriculation réutilise le véhicule et son '
  ...
  test(
      "ajouter deux fois le même service incrémente la quantité au lieu "
  ...
  test('addProduitLine ajoute une ligne produit distincte des services',
      () async {
    final prestation = await repository.createPrestationForImmatriculation(
      establishmentId: 'est-1',
  ...
  test('attachClient rattache le client à la prestation et au véhicule',
      () async {
    final prestation = await repository.createPrestationForImmatriculation(
      establishmentId: 'est-1',
    ...
    final updatedPrestation =
        await repository.watchPrestation(prestation.id).first;
    final updatedVehicule =
        await repository.watchVehicule(prestation.vehiculeId).first;
    expect(updatedPrestation!.clientId, client.id);
    expect(updatedVehicule!.clientId, client.id);
  });

  test('detachClient retire le client de la prestation uniquement',
      () async {
    final prestation = await repository.createPrestationForImmatriculation(
      establishmentId: 'est-1',
